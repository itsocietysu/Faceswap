...
def getFaceKeypoints(img, detector, predictor, dets=None, maxImgSizeForDetection=320):
    imgScale = 1
    scaledImg = img
    ...
    for det in dets:
        ...
        dlibShape = predictor(np.array(img), faceRectangle)
...
def getFaceTextureCoords(img, mean3DShape, blendshapes, idxs2D, idxs3D, detector, predictor):
    ...
    keypoints, dets = getFaceKeypoints(img, detector, predictor)
