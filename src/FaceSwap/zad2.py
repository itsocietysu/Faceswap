...
textureCoords = utils_fs.getFaceTextureCoords(textureImg, mean3DShape, blendshapes, idxs2D, idxs3D, detector, predictor)
renderer = FaceRendering.FaceRenderer(cameraImg, textureImg, textureCoords, mesh)

while True:
    cameraImg = cap.read()[1]
    cameraImg = cv2.resize(cameraImg, (640, 480))
    shapes2D = utils_fs.getFaceKeypoints(cameraImg, detector, predictor, maxImageSizeForDetection)
